    ...
    <div className="Sidebar hidden sm:block dark:text-gray-100 pt-2">
      <ul className="flex flex-col gap-y-4 pl-2 w-40 lg:w-60 pr-4">
        ...
        {genres.map((genre, index) => (
          <li key={genre} className={`flex justify-start items-center gap-x-2 lg:gap-x-3 ${mainGenre === genre && "font-bold"} hover:bg-gray-300 dark:hover:bg-zinc-700 rounded-md cursor-pointer`} onClick={() => onChangeGenre(genre)} >
